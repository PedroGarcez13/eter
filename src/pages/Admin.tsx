...
function Stat({ k, value, gold }: { k: string; value: string; gold?: boolean }) {
  return (
    <div className={`card-surface p-4 ${gold ? "bg-gold border-transparent" : ""}`}>
      <div className={`text-xs font-semibold uppercase tracking-wide ${gold ? "text-white/85" : "text-brand-blue2"}`}>{k}</div>
      <div className={`font-display text-3xl mt-1.5 leading-none ${gold ? "text-white" : "text-brand-blue"}`}>{value}</div>
    </div>
  );
}
